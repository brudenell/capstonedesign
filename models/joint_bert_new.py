...
#                pooling='mean', name='AlbertLayer')(bert_inputs)
    ...
    # add save_path arg JAEHAK
    def fit(self, X, Y, validation_data=None, epochs=5, batch_size=64, save_path):
        """
        X: batch of [input_ids, input_mask, segment_ids, valid_positions]
        """
        # checkpoint_path JAEHAK
        checkpoint_path = save_path + "/" + "joint_bert_model_epoch{epoch:04d}.h5"
        ...
        # add "shuffle = True" JAEHAK
        history = self.model.fit(X, Y, validation_data=validation_data,
                                 epochs=epochs, callbacks=[cp_callback], shuffle=True, batch_size=batch_size)
        ...
        # move save() here JAEHAK
        with open(os.path.join(save_path, 'params.json'), 'w') as json_file:
            json.dump(self.model_params, json_file)
        self.model.save(checkpoint_path.format(epoch=0))
